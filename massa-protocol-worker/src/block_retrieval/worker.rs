...
use massa_models::{
    block::{Block, BlockId, BlockSerializer, WrappedBlock, WrappedHeader},
    node::NodeId,
...
impl Worker {
    ...
    pub async fn run(
        ...
        let operation_ids: Vec<OperationId> = self
            .retrieve_block_op_list(&block_id, &header.content.operation_merkle_root)
            .await?;
        ...
        if !missing_ops.is_empty() {
            self.retrieve_missing_ops(&mut block_storage, missing_ops)
                .await?;
        }
        ...
        .map_err(|err| {
            BlockRetrieverError::Invalid(format!("could not build received block: {}", err))
        })?;
